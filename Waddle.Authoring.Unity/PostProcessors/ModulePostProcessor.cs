...
namespace Waddle.Authoring.Unity.PostProcessors
{
    public class ModulePostProcessor : AssetPostprocessor
    {
        ...
        private static string GenerateClass(ModuleDefinitionContainer moduleDefinition)
        {
            ...
            sb.AppendLine("{");
            sb.AppendLine(
                $"\tpublic class {moduleDefinition.name} : UnityEngine.MonoBehaviour, Waddle.Authoring.Unity.Baking.IModuleWrapper");
            sb.AppendLine("\t{");
            sb.AppendLine("\t\t[UnityEngine.SerializeField] private Waddle.Authoring.Unity.EntityContainer.ModuleWrapper _module;");
            sb.AppendLine();
            int index = 0;
